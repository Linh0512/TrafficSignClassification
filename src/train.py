...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Huấn luyện mô hình YOLOv12 cho nhận diện biển báo giao thông')
    parser.add_argument('--model', type=str, default=os.path.join(root_dir, 'models', 'yolo12n.pt'), 
                      help='Mô hình YOLOv12 muốn sử dụng (yolo12n.pt, yolo12s.pt, yolo12m.pt, yolo12l.pt, yolo12x.pt)')
...
def train(args):
    ...
    results = model.train(
        ...
        exist_ok=True,
        pretrained=True,
        optimizer='SGD',  # 'SGD', 'Adam', hoặc 'AdamW'
        lr0=1e-3,  # learning rate ban đầu (i.e. SGD=1E-2, Adam=1E-3)
        lrf=0.01,  # learning rate cuối cùng (lr0 * lrf)
        momentum=0.937,  # SGD momentum/Adam beta1
        weight_decay=0.0005,  # weight decay của optimizer
        warmup_epochs=3.0,  # số epochs warmup (fractions ok)
        warmup_momentum=0.8,  # initial momentum cho warmup
        warmup_bias_lr=0.1,  # initial bias lr cho warmup
